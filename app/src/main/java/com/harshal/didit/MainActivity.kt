...
class MainActivity : AppCompatActivity() {
    ...
    private fun setupRecyclerView() {
        try {
            ...
            binding.ongoingTasksRecyclerView.apply {
                ...
                setHasFixedSize(true) // Improves performance when item size is fixed
    ...
    private fun animateTaskSlideDown(fromRecyclerView: androidx.recyclerview.widget.RecyclerView, 
                                   toRecyclerView: androidx.recyclerview.widget.RecyclerView, 
                                   fromIndex: Int) {
        // Delay the animation to ensure the RecyclerView has updated
        fromRecyclerView.post {
            val fromViewHolder = fromRecyclerView.findViewHolderForAdapterPosition(fromIndex)
            if (fromViewHolder != null) {
                // Create a smooth slide down animation for completion
                fromViewHolder.itemView.animate()
                    .translationY(200f) // Slide down
                    .alpha(0f)
                    .setDuration(300)
                    .setInterpolator(android.view.animation.AccelerateInterpolator())
                    .withEndAction {
                        // After slide down animation, animate the addition with slide up
                        toRecyclerView.post {
                            val toViewHolder = toRecyclerView.findViewHolderForAdapterPosition(0)
                            toViewHolder?.let { holder ->
                                // Set initial state for smooth slide up entrance
                                holder.itemView.translationY = -200f
                                holder.itemView.alpha = 0f
                                
                                // Animate to final state with slide up
                ...
                // Fallback: just animate the addition if we can't find the removal view
                toRecyclerView.post {
                    val toViewHolder = toRecyclerView.findViewHolderForAdapterPosition(0)
                    toViewHolder?.let { holder ->
                        holder.itemView.translationY = -200f
    ...
    private fun animateTaskSlideUp(fromRecyclerView: androidx.recyclerview.widget.RecyclerView, 
                                 toRecyclerView: androidx.recyclerview.widget.RecyclerView, 
                                 fromIndex: Int) {
        // Delay the animation to ensure the RecyclerView has updated
        fromRecyclerView.post {
            val fromViewHolder = fromRecyclerView.findViewHolderForAdapterPosition(fromIndex)
            if (fromViewHolder != null) {
                // Create a smooth slide up animation for undo
                fromViewHolder.itemView.animate()
                    .translationY(-200f) // Slide up
                    .alpha(0f)
                    .setDuration(300)
                    .setInterpolator(android.view.animation.AccelerateInterpolator())
                    .withEndAction {
                        // After slide up animation, animate the addition with slide down
                        toRecyclerView.post {
                            val toViewHolder = toRecyclerView.findViewHolderForAdapterPosition(0)
                            toViewHolder?.let { holder ->
                                // Set initial state for smooth slide down entrance
                                holder.itemView.translationY = 200f
                                holder.itemView.alpha = 0f
                                
                                // Animate to final state with slide down
                                holder.itemView.animate()
                                    .translationY(0f)
                                    .alpha(1f)
                                    .setDuration(350)
    ...
    private fun showAddTaskDialog() {
        ...
        dialog.setOnShowListener {
            ...
            dialogBinding.animate()
                ...
                .scaleY(1f)
                .translationY(0f)
                .setDuration(200) // Faster animation for high refresh rate
            ...
            if (saveButton != null) {
                ...
                saveButton.setOnClickListener {
                    ...
                    if (taskName.isNotEmpty()) {
                        ...
                        // Animate button press with optimized timing for high refresh rate
                        saveButton.animate()
                            .scaleX(0.95f)
                            .scaleY(0.95f)
                            .setDuration(80) // Faster animation for high refresh rate
                            .setInterpolator(android.view.animation.AccelerateInterpolator())
                            .withEndAction {
                                saveButton.animate()
                                    .scaleX(1f)
                        ...
                        // Shake animation for empty input with optimized timing for high refresh rate
                        editText.animate()
                            .translationX(-10f)
                            .setDuration(40) // Faster animation for high refresh rate
                            .setInterpolator(android.view.animation.AccelerateInterpolator())
                            .withEndAction {
                                editText.animate()
                                    .translationX(10f)
    ...
    private fun addNewTask(taskName: String, notes: String = "") {
        ...
        // Temporary toast to verify task creation
        android.widget.Toast.makeText(this, "Task '$taskName' added successfully!", android.widget.Toast.LENGTH_SHORT).show()
    }
    ...
    private fun showClearAllTasksDialog() {
        // Create custom dialog layout
        val dialogBinding = layoutInflater.inflate(
            com.harshal.didit.R.layout.dialog_clear_all_confirmation,
            null
        )
        ...
        dialogBinding.findViewById<android.widget.Button>(com.harshal.didit.R.id.clearAllButton).setOnClickListener {
            clearAllTasks()
            dialog.dismiss()
        }
    ...
    private fun clearAllTasks() {
        ...
        TaskRepository.clearAllTasks(this)
        setupSectionHeaders()
